...
from ynsbase.embedded.pyyaml import yaml
...
'http://%pythonanywhere_username%.pythonanywhere.com/'\
    ...
    URL_PROJEKTCARD =\
'http://%pythonanywhere_username%.pythonanywhere.com/PROJEKTCARD'\
        .replace(
            '%pythonanywhere_username%',
            pythonanywhere_username
        )
...
and not '500 Internal Server Error' in response_text:
            is_online = True
            cards.append(response_text)
            PROJEKTCARD_dict = yaml.load(response_text)
